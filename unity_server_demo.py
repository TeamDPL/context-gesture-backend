...
# Selection state (for demo mode)
current_tool = None
counter = 0
threshold = 0  # 1~3 사이의 값
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    global current_tool, counter, threshold
    ...
    try:
        while True:
            ...
            if latest_gesture_embed is not None and latest_context_embed is not None and inventory_labels:
                # Check if we need to select a new tool
                # (when counter reaches threshold, or inventory changed, or no current tool)
                if counter >= threshold or current_tool not in inventory_labels:
                    # Select new tool and set new threshold (1~3)
                    current_tool = random.choice(inventory_labels)
                    threshold = random.randint(1, 3)
                    counter = 0
                    print(f"[Classifier] selected tool: {current_tool}")
                
                # Use current tool
                predicted_tool = current_tool
                counter += 1
                
                # Create dummy scores for logging (all zeros except selected one)
                scores = [0.0] * len(inventory_labels)
                if current_tool in inventory_labels:
...
# run server:
# uvicorn unity_server_demo:app --host 0.0.0.0 --port 8000
